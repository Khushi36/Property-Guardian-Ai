...
def natural_language_search(query_text: str, db: Session, session_id: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    try:
                ...
                if history:
                    ...
                    for msg in history:
                        if isinstance(msg, dict) and "role" in msg and "content" in msg:
                            history_parts.append(f"{str(msg['role']).title()}: {msg['content']}\n")
            ...
            try:
                ...
                for keyword in keywords_to_search:
                    try:
                        ...
                        logger.warning(f"Keyword search for '{keyword}' failed (likely API limits): {e}")
                        continue
                        
                    # Merge keyword results into main results
                    kw_docs = cast(Optional[List[List[str]]], keyword_results.get('documents'))
                    kw_metas = cast(Optional[List[List[Dict[str, Any]]]], keyword_results.get('metadatas'))
                    
                    if isinstance(kw_docs, list) and len(kw_docs) > 0 and isinstance(kw_docs[0], list):
                        for j, doc in enumerate(kw_docs[0]):
                            # Explicitly check for results structure to satisfy linter
                            docs_list = cast(Optional[List[List[str]]], results.get('documents'))
                            metas_list = cast(Optional[List[List[Dict[str, Any]]]], results.get('metadatas'))
                            dists_list = cast(Optional[List[List[float]]], results.get('distances'))
                            
                            if isinstance(docs_list, list) and len(docs_list) > 0 and isinstance(docs_list[0], list) and doc not in docs_list[0]:
                                docs_list[0].insert(0, str(doc))
                                if isinstance(metas_list, list) and len(metas_list) > 0 and isinstance(metas_list[0], list) and isinstance(kw_metas, list) and len(kw_metas) > 0:
                                    metas_list[0].insert(0, kw_metas[0][j])
                                if isinstance(dists_list, list) and len(dists_list) > 0 and isinstance(dists_list[0], list):
                                    dists_list[0].insert(0, 0.5)
                        logger.info(f"Keyword search for '{keyword}' found {len(kw_docs[0])} results")
        ...
        if isinstance(docs_raw, list) and len(docs_raw) > 0 and docs_raw[0]:
            ...
            if not relevant_indices and isinstance(raw_docs, list) and len(raw_docs) > 0:
                ...
                for i_idx, meta_val in enumerate(metadatas):
                    ...
                    for ridx in relevant_indices:
                        if ridx == i_idx:
                            ...
                            break
                    if is_relevant:
                        if isinstance(meta_val, dict) and 'property_id' in meta_val and meta_val['property_id'] != -1:
        ...
        try:
            ...
            if p_dist and p_dist != "null" and p_dist.lower() != "unknown":
                dist_props = db.query(sql_models.Property.id).filter(
            ...
            if not property_ids:
                villages = []
                try:
                    villages_raw = db.query(sql_models.Property.village).distinct().all()
                ...
                for v in villages:
                    if isinstance(v, str) and v.lower() in query_text.lower():
                        matched_props = db.query(sql_models.Property.id).filter(
                            sql_models.Property.village.ilike(f"%{v}%")
                        ).limit(5).all()
            ...
            long_ids = re.findall(r'\b\d{4,}\b', query_text)
            for eid in long_ids:
                prop_obj = db.query(sql_models.Property).filter(sql_models.Property.id == int(eid)).first()
                if prop_obj:
                    property_ids.add(prop_obj.id)
        except Exception as e:
            logger.warning(f"Failed to fetch fallback properties for query context: {e}")
        ...
        for pid_val in property_ids_list:
            if len(final_property_ids) >= 20: break
            final_property_ids.append(int(pid_val))
        
        if not final_property_ids and not raw_docs:
            return {"answer": "No relevant property documents found. Try uploading some property deeds first.", "sources": []}
        ...
        if final_property_ids_typed:
            ...
            properties = db.query(sql_models.Property).filter(
                sql_models.Property.id.in_(final_property_ids_typed)
            ).all()
            ...
            for txn in transactions:
                ...
                if txn.property:
                    plot = str(txn.property.plot_no) if txn.property.plot_no else "Not Specified"
                    village = str(txn.property.village) if txn.property.village else "Unknown Village"
                    district = str(txn.property.district) if txn.property.district else "Unknown District"
                db_context_parts.append(f"- Location: {district}, {village}, Plot: {plot} | Transaction: {seller_name} sold to {buyer_name} on {txn.registration_date}\n")
            ...
            for pid in final_property_ids_typed:
                ...
                if current_pid_val not in seen_props:
                    if current_pid_val in prop_map:
                        ...
                        plot_str = str(p_inst.plot_no) if p_inst.plot_no else "Not Specified"
                        village_str = str(p_inst.village) if p_inst.village else "Unknown Village"
                        district_str = str(p_inst.district) if p_inst.district else "Unknown District"
                        db_context_parts.append(f"- Location: {district_str}, {village_str}, Plot: {plot_str} (Status: Registered property, no transactions recorded)\n")
        else:
            ...
            if isinstance(raw_history_full, list):
                ...
                for h in extracted_history:
                    if isinstance(h, dict) and "role" in h and "content" in h:
                        msg_entry: Dict[str, Any] = {"role": str(h["role"]), "content": str(h["content"])}
                        if h.get("reasoning_details"):
                            msg_entry["reasoning_details"] = str(h["reasoning_details"])
                        history_msgs.append(msg_entry)
        
        logger.info(f"Query: {query_text} | Session: {session_id} | History Count: {len(history_msgs)}")
        if history_msgs:
            logger.debug(f"Last History Msg: {history_msgs[-1]['content']}")

        llm_gen: LLMClient = cast(LLMClient, llm_client)
        human_answer, reasoning_details = llm_gen.generate_response(context_text, query_text, history_messages=history_msgs)
        
        # 5. Save loop to History
        if session_id:
            chat_history.add_message(db, session_id, "user", query_text)
            chat_history.add_message(db, session_id, "assistant", human_answer, reasoning_details=reasoning_details)
...
def ai_to_sql_query(nl_query: str, db: Session) -> Any:
    ...
    if not params:
        return {"message": "Could not interpret query. Try asking naturally like 'show properties in Wakad'"}
    ...
    if params.get("seller_name") and params["seller_name"] != "null":
        ids = ["transactions t ON p.id = t.property_id", "people s ON t.seller_id = s.id"]
...
def execute_direct_sql_safe(sql_query: str, db: Session, user_id: Optional[int] = None, log_db: Optional[Session] = None) -> List[Dict[str, Any]]:
    """
