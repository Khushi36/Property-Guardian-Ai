...
def extract_metadata(file_path: str) -> dict:
    ...
    try:
        ...
        if not text_stripped:
            logger.warning("pypdf: PDF text seems empty. It might be an image-based PDF.")
        ...
        try:
            ...
            raise ValueError(f"Failed to extract text from PDF: {str(e)}. Install pdfplumber for fallback: pip install pdfplumber")
            ...
            try:
                ...
                logger.info(f"Tesseract OCR fallback succeeded: {len(text)} chars extracted.")
            except Exception as e3:
                raise ValueError(f"Failed to extract text from PDF with pypdf, pdfplumber, and Tesseract OCR: {str(e3)}")
    ...
    def find_val(label_pattern, extra_flags=re.IGNORECASE):
        ...
        val = match.group(1).strip() if match and match.lastindex == 1 and match.group(1) else "Unknown"
        
        # Cleanup specific prefixes often found in these docs
        for prefix in ["Name:", "Mr.", "Mrs.", "S/O", "D/O", "W/O", "Name :", "Mr ", "Mrs ", "Mrs.Name:"]:
            if isinstance(val, str) and isinstance(prefix, str) and val.lower().startswith(prefix.lower()):
    ...
    extracted = {
        ...
        "plot_no": find_val(r"(?:Other Description of the(?: Property)?.*?Khewat Number|Plot|Plot No|Plot Number|Gut No|Gat No|Survey No|Khewat No|Khewat Number|Khatauni Number|Khatauni No|Khewat)\.?"), 
        "house_no": find_val(r"(?:House|House No|Flat No|Apartment)\.?"),
        "seller_name": find_val(r"(?:Seller(?:.*?Name:)?|Transferor|Vendor|First Party)(?: Name)?", re.IGNORECASE | re.DOTALL), 
        "buyer_name": find_val(r"(?:Buyer(?:.*?Name:)?|Transferee|Purchaser|Second Party)(?: Name)?", re.IGNORECASE | re.DOTALL),
        "seller_aadhaar": find_val(r"Seller Aadhaar(?: Number)?|Aadhaar(?: No)?\.? (?:of Seller)?"),
        "seller_pan": find_val(r"Seller PAN(?: Number)?|PAN(?: No)?\.? (?:of Seller)?"),
        "buyer_aadhaar": find_val(r"Buyer Aadhaar(?: Number)?|Aadhaar(?: No)?\.? (?:of Buyer)?"),
        "buyer_pan": find_val(r"Buyer PAN(?: Number)?|PAN(?: No)?\.? (?:of Buyer)?"),
        "registration_date": find_val(r"Document Registration Date(?: :-)?|Registration Date(?: :-)?"),
    }
    ...
    for k, v in extracted.items():
        if k != "text_content" and isinstance(v, str) and v != "Unknown":
            ...
            if v_lower.startswith("of "): 
                ...
                off_idx_val = 0
                for oc in v:
                    if off_idx_val >= 3: off_chars.append(str(oc))
                    off_idx_val += 1
                ...
                the_idx_val = 0
                for tc in v:
                    if the_idx_val >= 4: the_chars.append(str(tc))
                    the_idx_val += 1
    ...
    critical_missing = (extracted["village"] == "Unknown" or extracted["district"] == "Unknown" or extracted["plot_no"] == "Unknown")
    ...
    if critical_missing or suspicious_length:
        logger.info(f"Weak extraction. Missing: {critical_missing}, Suspicious: {suspicious_length}. Text len: {len(text)}. Invoking LLM...")
        try:
...
def process_document(
    ...
    try:
        ...
        existing_doc = db.query(sql_models.Document).filter(
            sql_models.Document.file_hash == file_hash
        ).first()
        
        # Secondary check: same filename already processed
        if not existing_doc and getattr(file, 'filename', None):
            existing_doc = db.query(sql_models.Document).filter(
                sql_models.Document.file_path.contains(file.filename)
            ).first()
        
        if existing_doc:
            return {"status": "skipped", "message": "Document already exists or same filename was uploaded.", "document_id": existing_doc.id}
        ...
        file_path = os.path.join(settings.DOCUMENT_STORAGE_PATH, f"{file_hash}_{safe_filename}")
        ...
        if known_count < 2:
            ...
            logger.warning(f"Weak extraction for {getattr(file, 'filename', 'unknown')} (only {known_count}/4 fields). Doing partial ingestion.")
        ...
        logger.info(f"Extracted metadata for {getattr(file, 'filename', 'unknown')}: village={extracted.get('village')}, district={extracted.get('district')}, plot={extracted.get('plot_no')}")
        ...
        prop = db.query(sql_models.Property).filter_by(
        ...
        if not prop:
            candidates = db.query(sql_models.Property).filter_by(
                state=extracted['state'],
                district=extracted['district'],
                tehsil=extracted['tehsil'],
                village=extracted['village']
            ...
            for candidate in candidates:
                if normalize_property_details(candidate.plot_no) == norm_plot_no:
                    prop = candidate
                    logger.info(f"Normalized exact match found! '{raw_plot_no}' -> '{prop.plot_no}'")
                    break
        ...
        if not prop:
            prop = sql_models.Property(
                ...
                tehsil=extracted['tehsil'],
                village=extracted['village'],
                plot_no=extracted['plot_no'], # Storing the raw one from this doc, next time it will match fuzzy
        ...
        def get_or_create_person(name: str, aadhaar: str = None, pan: str = None, db: Session = None) -> sql_models.Person:
            person = None
            
            # 1. Unique ID Match (The most reliable)
            if aadhaar and aadhaar != "Unknown":
                person = db.query(sql_models.Person).filter_by(aadhaar_number=aadhaar).first()
            ...
            if not person:
                # Exact Name Match
                person = db.query(sql_models.Person).filter(
                    sql_models.Person.name.ilike(name)
                ).first()
            ...
            if not person:
                ...
                if all_people:
                    ...
                    if best_match and best_match[1] > 90:
                        ...
                        person = db.query(sql_models.Person).filter_by(name=matched_name).first()
                        logger.info(f"Fuzzy match found for Person: '{name}' -> '{matched_name}'")
