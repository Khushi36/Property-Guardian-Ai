...
def detect_fraud(db: Session):
    ...
    subquery = (
        select(
            ...
            func.lag(t1.buyer_id)
            .over(partition_by=t1.property_id, order_by=(t1.registration_date, t1.id))
            .label("prev_buyer_id"),
        )
    ).subquery()
    ...
    double_sell_stmt = (
        ...
        .join(
            sql_models.Property,
            sql_models.Transaction.property_id == sql_models.Property.id,
        )
        .join(
            sql_models.Person, sql_models.Transaction.seller_id == sql_models.Person.id
        )
        .group_by(
            sql_models.Transaction.property_id,
            sql_models.Transaction.seller_id,
