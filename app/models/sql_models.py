from sqlalchemy import Column, Integer, String, Date, ForeignKey, DateTime, UniqueConstraint, Boolean, JSON
...
class Property(Base):
    ...
    __table_args__ = (
        UniqueConstraint('state', 'district', 'tehsil', 'village', 'plot_no', 'house_no', name='uix_property_location'),
    )
...
class Person(Base):
    ...
    pan_number = Column(String, unique=True, nullable=True) # Assuming PAN as a unique identifier for simplicity
    aadhaar_number = Column(String, unique=True, nullable=True) # Or Aadhaar
    role = Column(String) # 'seller', 'buyer', or 'official' - though roles change per transaction
...
class Document(Base):
    ...
    file_hash = Column(String, unique=True, nullable=False) # SHA256 hash to detect duplicate files
