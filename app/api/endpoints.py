...
async def get_current_user(
    ...
    user = (
        db.query(sql_models.User)
        .filter(func.lower(sql_models.User.email) == token_data.email.lower())
        .first()
    )
    if user is None:
        raise credentials_exception
...
async def login_for_access_token(
    ...
    user = (
        db.query(sql_models.User)
        .filter(func.lower(sql_models.User.email) == form_data.username.lower())
        .first()
    )
    if not user:
...
def create_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    db_user = (
        db.query(sql_models.User)
        .filter(func.lower(sql_models.User.email) == email_lower)
        .first()
    )
    if db_user:
...
def confirm_password_reset(
    request: Request, data: schemas.PasswordResetConfirm, db: Session = Depends(get_db)
):
...
async def auth_google_callback(code: str, db: Session = Depends(get_db)):
    ...
    return {
        "message": "Google Login Callback received. Configure Client ID/Secret to activate."
    }
...
def ingest_document(
    ...
    for file in files:
        ...
        if not file.filename.lower().endswith(".pdf"):
            errors.append(
                f"{file.filename}: Not a PDF file. Only PDF files are accepted."
            )
            continue
    ...
    if not results and errors:
        raise HTTPException(
            status_code=400, detail=f"Failed to process files: {'; '.join(errors)}"
        )
...
def query_direct_sql(
    ...
    return query_service.execute_direct_sql_safe(
        request.query, db_readonly, user_id=current_user.id, log_db=db_write
    )
        ...
        "data": [
            {
                "id": t.id,
                "property": (
                    f"{t.property.village}, Plot {t.property.plot_no}"
                    if t.property
                    else "Unknown"
                ),
